...
describe('plugin: prefixId SMIL', () => {
  describe('without offsets', () => {
    ...
    test('x.end', () => {
      expect(result.content).toContain('begin="icon-test-b.end"')
    })
    test('x.begin', () => {
      expect(result.content).toContain('begin="icon-test-a.begin"')
    })
  })
  describe('with offsets: x.end-0.5s', () => {
    ...
    const result = bakeIcon({ name: 'icon-test', content: svg }, options)
    test('x.end-0.5s', () => {
      expect(result.content).toContain('begin="icon-test-b.end-0.5s"')
    })
    test('x.begin+0.1s', () => {
      expect(result.content).toContain('begin="icon-test-a.begin+0.1s"')
    })
  })
  describe('mix with offsets and without ', () => {
    ...
    test('x.begin;x.end-0.5s', () => {
      expect(result.content).toContain('begin="icon-test-b.begin; icon-test-b.end-0.5s"')
    })
    test('x.end;x.begin+0.1s', () => {
      expect(result.content).toContain('begin="icon-test-a.end; icon-test-a.begin+0.1s"')
    })
  })
})
